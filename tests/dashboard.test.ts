...
test("navbar is visible", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: "Films", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: "Favourites", exact: true }),
  ).toBeVisible();
});
...
test("stats cards are visible", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: /Characters Heroes/ }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: /Films The complete/ }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: /Favourites Your personally/ }),
  ).toBeVisible();
});
